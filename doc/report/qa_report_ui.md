# 📊 QA Report — HermesTranslate UI (Tester + Monitor) — DEPRECATED

> **DEPRECATED (2026-07-01):** `tester.html` และ `monitor.html` ถูกลบออกจากโปรเจคแล้ว รายงานนี้เก็บไว้เป็นบันทึกประวัติศาสตร์ QA เท่านั้น Endpoint API ทั้งหมดยังคงทำงานตามปกติ

| Item | Value |
|------|-------|
| **Phase** | Add-on — Web UI (Translation Tester + Queue Monitor) — UI files removed |
| **Version** | 1.0.0 |
| **Date** | 1 กรกฎาคม 2026 (Updated: 1 กรกฎาคม 2026 — UI files removed) |
| **Status** | PASS (20/20 tests) — 12 tests deprecated due to UI removal |
...
|--------|-------------|--------|----------|
| ~~UI-01 (Tester)~~ | หน้าเว็บกรอกข้อความไทย → ยิง API → แสดง Task ID *(removed 2026-07-01)* | ~~PASS~~ | `static/tester.html` — ถูกลบแล้ว |
| ~~UI-02 (Monitor)~~ | หน้าเว็บแสดง Queue Stats แบบ Real-time (Polling 2s) *(removed 2026-07-01)* | ~~PASS~~ | `static/monitor.html` — ถูกลบแล้ว |
...
| ~~4~~ | ~~`GET /static/tester.html` → 200~~ *(deprecated — file removed)* | ~~PASS~~ |
| ~~5~~ | ~~`GET /static/monitor.html` → 200~~ *(deprecated — file removed)* | ~~PASS~~ |
...
| ~~9~~ | ~~tester.html → validation ช่องว่าง → error message~~ *(deprecated)* | ~~PASS~~ |
| ~~10~~ | ~~tester.html → API down → error message (ไม่ crash หน้า)~~ *(deprecated)* | ~~PASS~~ |
| ~~11~~ | ~~tester.html → Ctrl+Enter submit~~ *(deprecated)* | ~~PASS~~ |
| ~~12~~ | ~~tester.html → แสดง Task ID card หลังส่ง~~ *(deprecated)* | ~~PASS~~ |
| ~~13~~ | ~~monitor.html → 6-card dashboard + real-time polling~~ *(deprecated)* | ~~PASS~~ |
| ~~14~~ | ~~monitor.html → Consumer active (consumers=1, สีเขียว)~~ *(deprecated)* | ~~PASS~~ |
...
| ~~6~~ | ~~`tester.html`~~ | ~~`static/tester.html`~~ | ~~4,332 B~~ | ~~Translation Tester UI (single-file) — removed 2026-07-01~~ |
| ~~7~~ | ~~`monitor.html`~~ | ~~`static/monitor.html`~~ | ~~5,742 B~~ | ~~Queue Monitor Dashboard (single-file) — removed 2026-07-01~~ |

**Total:** 4 modified + ~~2 new~~ (removed) + 1 dependency

---

## Architecture Notes

> **หมายเหตุ (2026-07-01):** Architecture diagrams และ data flows ด้านล่างอ้างอิง `tester.html` และ `monitor.html` ซึ่งถูกลบแล้ว — เก็บไว้เป็นบันทึกประวัติศาสตร์
...
1. ~~**Single-file HTML (no framework):** ทั้ง tester.html และ monitor.html เป็น vanilla HTML+CSS+JS ไฟล์เดียว — ไม่มี npm/build step, เปิดด้วย browser ได้ทันที~~ *(UI files removed 2026-07-01)*
2. **CORS open (dev mode):** `allow_origins=["*"]` — เหมาะสำหรับ development/internal tool, ควร restrict ใน production
3. ~~**Polling over WebSocket:** Monitor ใช้ polling 2 วินาที (ไม่ใช่ WebSocket) — เพียงพอสำหรับ internal dashboard ที่มีผู้ใช้ 1-2 คน, ลด complexity~~ *(monitor.html removed)*
...
5. ~~**Backend-first:** `/queue/stats` เป็น endpoint REST ปกติ — monitor.html กับ tool อื่นๆ ใช้ endpoint เดียวกันได้~~ *(monitor.html removed — `/queue/stats` endpoint ยังคงอยู่)*

### Known Limitations

- CORS allow_origins=["*"] — เหมาะ dev เท่านั้น ควร restrict ใน production
- ~~Monitor polling interval fixed 2s — ไม่สามารถปรับจาก UI~~ *(monitor.html removed)*
- ~~tester.html แสดงเฉพาะ Task ID — ไม่ auto-poll status~~ *(tester.html removed)*
...
- ~~tester.html + monitor.html รวม ~10 KB — โหลดเร็ว ไม่มี external dependency~~ *(UI files removed 2026-07-01)*
