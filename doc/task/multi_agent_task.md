...
1. [ ] สร้างไฟล์ `mq_consumer.py` ใช้ไลบรารี `aio-pika` 
2. [ ] กำหนดให้รันแบบวนลูป (Event loop) รอรับข้อความ เมื่อได้ข้อความ ให้ดึง `task_id` และ `text` ออกมา
3. [ ] **Definition of Done:** เมื่อลองรัน `python mq_consumer.py` มันจะต้องดึงข้อความที่ส่งค้างไว้ในคิว (จาก Phase 1) ออกมาปริ้นท์ (Print) โชว์บนหน้าจอได้

### Task 2: ประกอบร่างระบบ Multi-Agent
**คำอธิบาย:** สร้าง Agent 3 ตัวเพื่อรับส่งงานกัน
1. [ ] **Main Agent:** เขียนโค้ดรับ `text` แล้วไปดึงกฎจาก `rule_engine` (จาก Phase 2) นำมาแทรกใส่ `instruction.md` (ใช้ `jinja2`)
2. [ ] **Translate Agent:** นำไฟล์ MD Template ที่เติมข้อมูลแล้ว ยิงคำสั่งไปหา LLM (เช่น OpenAI API) เพื่อให้ทำการแปล
3. [ ] **Validate Agent:** นำผลลัพธ์การแปล กลับไปถาม LLM อีกรอบ โดยอิงจากกฎเดิม ว่านักแปลทำงานถูกต้องตามกฎหรือไม่
4. [ ] **Definition of Done:** ระบบสามารถรันต่อเนื่องตั้งแต่ Main -> Translate -> Validate และได้ผลลัพธ์การประเมินสุดท้ายออกมา

### Task 3: สร้างระบบ Fail-safes (Retry Logic)
**คำอธิบาย:** ระบบป้องกันความพินาศหาก AI แปลผิด
1. [ ] เขียนลอจิกในฝั่ง Validate Agent: หากผลประเมินคือ "ไม่ผ่าน" ให้วนลูปกลับไปสั่ง Translate Agent ทำงานใหม่
2. [ ] สร้างตัวนับ (Counter) จำวนครั้งที่แก้ไข
3. [ ] หากครบ 3 ครั้งแล้วยังไม่ผ่าน ให้หยุดการทำงาน แล้วอัปเดตสถานะของ `task_id` ในฐานข้อมูลเป็น `Failed / Manual Review`
4. [ ] **Definition of Done:** จำลองให้แปลผิดเสมอ ระบบจะต้องวนทำงานแค่ 3 รอบแล้วหยุดทันที
